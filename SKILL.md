...
- If clicking login does not submit successfully and the real rendered page shows no blocking exception, press `Enter` once as the standard submit fallback.
- Do **not** conclude slider/captcha exists from snapshot/DOM text.
- Only if the real rendered page clearly shows a slider/captcha layer and it actually blocks the flow should it be treated as an exception.
- Do not stop for manual intervention unless that exception is visibly shown on the real rendered page.
- Use `scripts/login_policy.py` as the canonical login policy reference and login execution entry.
