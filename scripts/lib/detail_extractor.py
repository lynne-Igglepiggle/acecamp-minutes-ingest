...
() => {
  ...
  const tags = (() => {
    ...
    if (!container) return [];
    const vals = Array.from(container.querySelectorAll('.tag-width, .hashtag, span, div'))
      .map(el => txt(el))
      .filter(Boolean);
    return Array.from(new Set(vals.filter(t => t.length <= 20)));
  })();
...
'''
    ...
    data['tags'] = _dedup_keep_order(data.get('tags', []))
    return data
